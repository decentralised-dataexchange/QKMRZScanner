...
public protocol QKMRZScannerViewDelegate: AnyObject {
    ...
    func mrzScannerView(_ mrzScannerView: QKMRZScannerView, didChangeBrightness isDark: Bool)
}

public extension QKMRZScannerViewDelegate {
    func mrzScannerView(_ mrzScannerView: QKMRZScannerView, didChangeBrightness isDark: Bool) {}
}
...
public class QKMRZScannerView: UIView {
    ...
    fileprivate var lastBrightnessIsDark: Bool?
    fileprivate var brightnessCheckCounter = 0
    ...
    public func startScanning() {
        ...
        DispatchQueue.global(qos: .userInitiated).async { [weak self] in
            ...
            DispatchQueue.main.async { [weak self] in
                ...
                self?.cutoutView.startScanLineAnimation()
            }
        }
    }
    ...
    public func stopScanning() {
        ...
        DispatchQueue.main.async { [weak self] in
            self?.cutoutView.stopScanLineAnimation()
        }
    }
    ...
    public var isTorchActive: Bool {
        guard let videoDevice = AVCaptureDevice.default(for: .video) else { return false }
        return videoDevice.isTorchActive
    }
...
extension QKMRZScannerView: AVCaptureVideoDataOutputSampleBufferDelegate {
    public func captureOutput(_ output: AVCaptureOutput, didOutput sampleBuffer: CMSampleBuffer, from connection: AVCaptureConnection) {
        ...
        // Brightness check every 15 frames to avoid overhead
        brightnessCheckCounter += 1
        if brightnessCheckCounter % 15 == 0 {
            let luminance = CIImage(cvImageBuffer: imageBuffer).averageLuminance
            let isDark = luminance < 0.25
            if isDark != lastBrightnessIsDark {
                lastBrightnessIsDark = isDark
                DispatchQueue.main.async { [weak self] in
                    guard let self = self else { return }
                    self.delegate?.mrzScannerView(self, didChangeBrightness: isDark)
                }
            }
        }

        guard let cgImage = imageBuffer.cgImage else {
            return
        }
